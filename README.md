...
Build65 closed as an **equivalence PASS but non-promoted performance experiment**. On the qualified Go 1.26.0 host the complete nine-photo matrix matched Build64 exactly, including every deep seed/evaluation/bank/qualification/decode/list-frame count and all payload/HMAC outcomes. Build65 used 8 seed workers, but B/mild still required **828036 ms** while logically trying **691 decode candidates / 2,120,047 list frames**.
...
Build59 is now a completed diagnostic-only **third pair-escape** study. The qualified Go 1.26.0 host artifact closes B/mild as `third-pair-qualified-gain`: 275 Build58 sibling parents include 50 proposal-local states and produce 180 retained third-pair states; 175 are qualified and none authenticate. On target candidate 10 the best qualified third-pair state reaches **25.407 px**, improving its exact **27.071 px** parent by **1.664 px** while proposal remains improved. B/angle target candidate 18 remains `third-pair-not-triggered`; the full B/angle Build59 bank contains 914 third-pair states, 0 qualified and 0 authenticated.

Build59 did not oracle-select the 26.149 px state. It reproduced every Build58 sibling, probed all 16 independent +/-1px coordinate neighbors from that exact sibling using proposal only, and applied the established bounded 112-combination pair stencil only when the sibling was a proposal-local maximum. At most eight pair escapes were retained per local sibling, ordered solely by proposal. The complete B/mild+B/angle bank was frozen before held-out/full-pilot qualification or diagnostic HMAC; SIFT/reference geometry remained post-hoc only.

The command is `v4-diagnose-phone-third-pair-escape`; see [`docs/V4_BUILD59_THIRD_PAIR_ESCAPE.md`](docs/V4_BUILD59_THIRD_PAIR_ESCAPE.md). Build59 changed no production path.

## What v0.3.0-build58 adds

Build58 is now a completed diagnostic-only **post-second-pair continuation sibling-stencil** study. Its qualified Go 1.26.0 host artifact closes B/mild as `second-pair-sibling-qualified-gain`. The preceding Build57 artifact had established that target candidate 10 retains 65 second-pair parents and 154 continuation states; 147 target continuation states qualify, none authenticate, and the best qualified intermediate reaches **26.968 px** post-hoc oracle error, `-7.080 px` from the original 34.049 px seed. B/angle target candidate 18 remains `second-pair-continuation-not-triggered`; its complete Build57 continuation bank has 347 states, none qualified or authenticated.

On the qualified host, Build58 produces 275 B/mild sibling states, 271 qualified and 0 authenticated. Target candidate 10 reaches a qualified **26.149 px** sibling from the 26.968 px Build57 parent while proposal improves from `0.276536` to `0.285435`. B/angle produces 542 sibling states, none qualified or authenticated. The generation remained proposal-only and the bank was frozen before qualification/HMAC; oracle geometry was post-hoc.
...
Build57 is now a completed diagnostic-only **post-second-pair continuation** study. On the qualified Go 1.26.0 host, B/mild target candidate 10 produces 154 continuation states from 65 retained second-pair parents; 147 are qualified, none authenticate, and the best qualified continuation reaches 26.968 px. Later proposal-improving states on the same branch move away from that geometric minimum, leaving an order-dependence question for Build58. B/angle target remains not triggered.

The command is `v4-diagnose-phone-sibling-pair-continue`; see [`docs/V4_BUILD57_SECOND_PAIR_CONTINUATION.md`](docs/V4_BUILD57_SECOND_PAIR_CONTINUATION.md). Build57 changed no production path.
...
The qualified-host Build56 artifact confirms the design probe. On the B/mild target, 16 of 91 siblings are proposal-local and emit 65 retained second-pair states. The best qualified state is second-pair rank 2 (`dimension 2 -1 px` + `dimension 4 +1 px`): proposal rises `0.266832 -> 0.273793`, post-hoc oracle error falls `29.778 -> 28.514 px`, qualification remains true and HMAC remains false. Across the complete bank, B/mild freezes 68 second-pair states (51 qualified, 0 authenticated); B/angle freezes 176 (0 qualified, 0 authenticated), while target candidate 18 remains `sibling-pair-not-triggered`.
...
Build54 reproduces that blind Build53 pair bank and then continues **every retained pair state** with a fixed 1px proposal-only coordinate descent for at most eight passes. Every accepted intermediate is retained; the greedy endpoint is not privileged. The complete root/pair/continuation bank is frozen before held-out/full-pilot qualification or diagnostic HMAC, and SIFT/reference geometry remains private and post-hoc.
...
Build53 is a diagnostic-only **coupled pair-escape optimizer study**. The qualified Build52 host run classified B/mild as `optimizer-partial-gain`: the 34.049 px target seed has a retained fine state at 31.747 px and a held-out-qualified retained state at 32.442 px, but HMAC remains false. B/angle is `optimizer-no-gain` and remains a distant false-basin control.

Build53 keeps the unchanged Build50/51/52 top4-per-side-pair seed bank and unchanged Build41 proposal objective. From each seed it retains the proposal-only 2px accepted-state path. Every retained root is probed with the complete +/-1px single-coordinate stencil. **Only roots with zero single-coordinate proposal improvements** trigger a bounded coupled +/-1px two-coordinate scan (112 deterministic combinations); at most eight proposal-improving pair states per root are retained, ranked only by proposal. The complete root/pair geometry bank is frozen before held-out/full-pilot qualification or diagnostic HMAC, and SIFT/reference geometry remains private and post-hoc.

The new command is `v4-diagnose-phone-pair-escape`; the private host study is `make v4-build53-phone-pair-escape-diagnostic`. See [`docs/V4_BUILD53_PAIR_ESCAPE.md`](docs/V4_BUILD53_PAIR_ESCAPE.md).
...
Build52 is a diagnostic-only **fine-first optimizer restart / accepted-state retention** study. Build51 showed that the B/mild seed has a proposal-compatible local improvement (`corner-2-x +2`: proposal `+0.006966`, oracle `34.049 -> 32.442 px`) that the standard `16, 8, 4, 2, 1` schedule never evaluates from the untouched seed because earlier coarse accepted moves have already changed the state.

The new `v4-diagnose-phone-restart` command keeps the exact Build50/51 top4-per-side-pair seeds and unchanged proposal score, then compares the ordinary Build41 coarse-to-fine endpoint against an independent `2 px -> 1 px` restart from each untouched seed. The fine restart retains the seed plus every accepted proposal-improving intermediate state. The full geometry bank is frozen before held-out/full-pilot qualification; the key/HMAC is used only after qualification, and SIFT/reference error remains private and post-hoc. See `docs/V4_BUILD52_FINE_RESTART.md`.
...
Build51 is a completed diagnostic-only **local proposal-surface and refinement-trajectory observability** study. The qualification-host result classifies B/mild as `optimizer-opportunity`: its 34.049 px target seed ends at 43.168 px under the ordinary coarse-to-fine refinement, yet the fixed stencil contains `corner-2-x +2`, which improves proposal by `+0.006966` and independent oracle error by `-1.607 px` to 32.442 px. The trace also visits a rejected 25.172 px state. B/angle has no better-both stencil point and is classified `proposal-surface-misaligned`.

The `v4-diagnose-phone-surface` command records every actually evaluated +/- coordinate move of the existing Build41 refiner and a fixed local stencil around every blind top4 seed. All trace/stencil geometry is generated from proposal evidence only and frozen before held-out annotation; SIFT/reference analysis remains private and post-hoc. See `docs/V4_BUILD51_LOCAL_SURFACE.md`.
