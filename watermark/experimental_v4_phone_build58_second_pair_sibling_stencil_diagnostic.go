...
const (
	experimentalV4PhoneBuild58SeedsPerPair = experimentalV4PhoneBuild56SeedsPerPair
)

// ExperimentalV4PhoneBuild58PairEscapeState is one proposal-improving coupled
...
type ExperimentalV4PhoneBuild58PairEscapeState struct {
	ExperimentalV4PhoneBuild53PairState
	ContinuationEvaluations int                                                   `json:"continuation_evaluations"`
	ContinuationStates      []ExperimentalV4PhoneBuild58PostPairContinuationState `json:"continuation_states,omitempty"`
}

// ExperimentalV4PhoneBuild58PostPairContinuationState is one proposal-improving 1px
...
// attached only after the complete Build58 bank has been frozen.
type ExperimentalV4PhoneBuild58PostPairContinuationState struct {
...
// ExperimentalV4PhoneBuild58SiblingState preserves one frozen Build55 sibling
// and records the proposal-only local-max probe plus any retained pair escapes.
type ExperimentalV4PhoneBuild58SiblingState struct {
	Rank              int                                         `json:"rank"`
	...
	PairStates        []ExperimentalV4PhoneBuild58PairEscapeState `json:"pair_states,omitempty"`
}

type ExperimentalV4PhoneBuild58ContinuationState struct {
	Parent   ExperimentalV4PhoneBuild55ContinuationState `json:"parent"`
	Siblings []ExperimentalV4PhoneBuild58SiblingState    `json:"siblings,omitempty"`
}

type ExperimentalV4PhoneBuild58Branch struct {
	PairRank           int                                           `json:"pair_rank"`
	ParentPair         ExperimentalV4PhoneBuild53PairState           `json:"parent_pair"`
	ContinuationStates []ExperimentalV4PhoneBuild58ContinuationState `json:"continuation_states,omitempty"`
}

type ExperimentalV4PhoneBuild58Root struct {
	...
	Branches          []ExperimentalV4PhoneBuild58Branch `json:"branches,omitempty"`
}

type ExperimentalV4PhoneBuild58Candidate struct {
	...
	Roots              []ExperimentalV4PhoneBuild58Root `json:"roots,omitempty"`
}

type ExperimentalV4PhoneBuild58Report struct {
	...
	Candidates                          []ExperimentalV4PhoneBuild58Candidate `json:"candidates,omitempty"`
}

type experimentalV4PhoneBuild58BlindPostPairContinuation struct {
	base         experimentalV4PhoneBuild55BlindContinuationState
	siblingEvals int
	siblings     []experimentalV4PhoneBuild55BlindSiblingState
}

type experimentalV4PhoneBuild58BlindSecondPairBranch struct {
	pair   experimentalV4PhoneBuild53BlindPairState
	evals  int
	states []experimentalV4PhoneBuild58BlindPostPairContinuation
}

type experimentalV4PhoneBuild58BlindSibling struct {
	base            experimentalV4PhoneBuild55BlindSiblingState
	singleEvals     int
	singleImproving int
	pairEvals       int
	pairImproving   int
	pairs           []experimentalV4PhoneBuild58BlindSecondPairBranch
}

type experimentalV4PhoneBuild58BlindContinuation struct {
	base     experimentalV4PhoneBuild55BlindContinuationState
	siblings []experimentalV4PhoneBuild58BlindSibling
}

type experimentalV4PhoneBuild58BlindBranch struct {
	pair   experimentalV4PhoneBuild53BlindPairState
	evals  int
	states []experimentalV4PhoneBuild58BlindContinuation
}

type experimentalV4PhoneBuild58BlindRoot struct {
	root              experimentalV4PhoneBuild52BlindState
	singleEvaluations int
	singleImproving   int
	pairEvaluations   int
	pairImproving     int
	branches          []experimentalV4PhoneBuild58BlindBranch
}

// ExperimentalV4PhoneBuild58Diagnose is research-only. It reproduces the full
// Build57 blind bank and then evaluates the complete independent +/-1px sibling
// stencil around every retained post-second-pair continuation state. Every
// sibling is generated from the identical frozen parent state and is retained
// only when it improves the unchanged proposal objective. The entire expanded
// B/mild+B/angle geometry bank is frozen before held-out/full-pilot
// qualification, protected-data decode, HMAC or oracle use.
func ExperimentalV4PhoneBuild58Diagnose(src image.Image, key []byte, cw, ch int) (ExperimentalV4PhoneBuild58Report, error) {
	var report ExperimentalV4PhoneBuild58Report
	if src == nil {
	...
	report.SeedsPerPair = experimentalV4PhoneBuild58SeedsPerPair
	...
	seeds := experimentalV4PhoneBuild48SelectSeeds(frozen, experimentalV4PhoneBuild58SeedsPerPair)
	...
	type blindCandidate struct {
		...
		roots    []experimentalV4PhoneBuild58BlindRoot
	}
	...
	for _, seed := range seeds {
		...
		bc := blindCandidate{seed: seed, baseline: baseline, roots: make([]experimentalV4PhoneBuild58BlindRoot, 0, len(roots))}
		for _, root := range roots {
			...
			br := experimentalV4PhoneBuild58BlindRoot{root: root, singleEvaluations: singleEvals, singleImproving: singleImproving}
			if singleImproving == 0 {
				...
				br.branches = make([]experimentalV4PhoneBuild58BlindBranch, 0, len(pairs))
				for _, pair := range pairs {
					...
					bb := experimentalV4PhoneBuild58BlindBranch{pair: pair, evals: contEvals, states: make([]experimentalV4PhoneBuild58BlindContinuation, 0, len(cont))}
					for _, cs := range cont {
						...
						outCS := experimentalV4PhoneBuild58BlindContinuation{base: cs, siblings: make([]experimentalV4PhoneBuild58BlindSibling, 0, len(sibs))}
						for _, sib := range sibs {
							se, si := experimentalV4PhoneBuild53SingleProbe(plane, pilot, cw, ch, anchor, sib.hyp, 0)
							report.SiblingLocalProbeEvaluations += se
							outS := experimentalV4PhoneBuild58BlindSibling{base: sib, singleEvals: se, singleImproving: si}
							if si == 0 {
								...
								outS.pairs = make([]experimentalV4PhoneBuild58BlindSecondPairBranch, 0, len(ps))
								for _, p := range ps {
									...
									outPair := experimentalV4PhoneBuild58BlindSecondPairBranch{pair: p, evals: ce, states: make([]experimentalV4PhoneBuild58BlindPostPairContinuation, 0, len(cont2))}
									for _, c2 := range cont2 {
										...
										outPair.states = append(outPair.states, experimentalV4PhoneBuild58BlindPostPairContinuation{base: c2, siblingEvals: se2, siblings: sibs2})
									}
	...
	for i, bc := range blind {
		...
		c := ExperimentalV4PhoneBuild58Candidate{
		...
		for ri, br := range bc.roots {
			...
			outRoot := ExperimentalV4PhoneBuild58Root{RootIndex: ri, Root: root, SingleEvaluations: br.singleEvaluations, SingleImproving: br.singleImproving, CoordinateLocal: br.singleImproving == 0, PairEvaluations: br.pairEvaluations, PairImproving: br.pairImproving, PairRetained: len(br.branches)}
			for pi, bb := range br.branches {
				...
				outBranch := ExperimentalV4PhoneBuild58Branch{PairRank: pi + 1, ParentPair: parentPair}
				for _, bcs := range bb.states {
					...
					outCS := ExperimentalV4PhoneBuild58ContinuationState{Parent: parentState}
					for sri, bs := range bcs.siblings {
						...
						outS := ExperimentalV4PhoneBuild58SiblingState{Rank: sri + 1, Parent: parentSibling, SingleEvaluations: bs.singleEvals, SingleImproving: bs.singleImproving, CoordinateLocal: bs.singleImproving == 0, PairEvaluations: bs.pairEvals, PairImproving: bs.pairImproving}
						for pri, bp := range bs.pairs {
							...
							outPair := ExperimentalV4PhoneBuild58PairEscapeState{ExperimentalV4PhoneBuild53PairState: ps, ContinuationEvaluations: bp.evals}
							for _, bcont := range bp.states {
								...
								outCont := ExperimentalV4PhoneBuild58PostPairContinuationState{ExperimentalV4PhoneBuild55ContinuationState: cs, SiblingEvaluations: bcont.siblingEvals}
								for si, bs2 := range bcont.siblings {
									sq2, _, sok2 := experimentalV4PhoneBuild41Qualify(plane, work, pilot, cw, ch, bs2.hyp, 0)
									ss2 := experimentalV4PhoneBuild55SiblingFromQualified(si+1, bs2, sq2, sok2, seedSource, srcBounds, work)
									if sok2 {
									...
									outCont.SiblingStates = append(outCont.SiblingStates, ss2)
								}
