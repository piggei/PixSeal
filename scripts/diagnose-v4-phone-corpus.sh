...
OUTPUT_DIR="${V4_PHONE_DIAGNOSTIC_DIR:-v4-phone private/build40-diagnostics}"
...
tsv="$OUTPUT_DIR/build40-phone-matrix.tsv"
md="$OUTPUT_DIR/build40-phone-matrix.md"
...
{
    echo '# PixSeal Build40 strength-48 phone diagnostic matrix'
    echo
    echo "Generated from the unchanged Build40 phone decoder with diagnostic-only telemetry. Canonical carrier: ${CANONICAL_WIDTH}x${CANONICAL_HEIGHT}."
    echo
    ...
    echo
    echo 'Interpretation discipline: controls must reject; marked captures count as recovered only when the exact payload authenticates. Pilot/Hamming telemetry is diagnostic and must not be used to rank geometry.'
} > "$md"

rm -f "$OUTPUT_DIR/logs/"*.payload.bin

echo "Build40 phone diagnostic matrix written to:"
echo "  $tsv"
echo "  $md"
