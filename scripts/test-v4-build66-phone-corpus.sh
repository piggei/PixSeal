...
OUTPUT_DIR="${V4_PHONE_BUILD66_DIAGNOSTIC_DIR:-v4-phone private/build66-diagnostics}"
...
PHONE_TIMEOUT="${V4_PHONE_BUILD66_TIMEOUT:-86400}"
BUILD65_BASELINE_TSV="${V4_PHONE_BUILD65_BASELINE_TSV:-v4-phone private/build65-diagnostics/build65-phone-matrix.tsv}"
...
[[ "$PHONE_TIMEOUT" =~ ^[0-9]+$ ]] || { echo "error: V4_PHONE_BUILD66_TIMEOUT must be an integer number of seconds" >&2; exit 1; }
...
specs=(
    ...
    "phone-b-mild.jpg|B|required-build66|$MESSAGE_B"
...
tsv="$OUTPUT_DIR/build66-phone-matrix.tsv"
md="$OUTPUT_DIR/build66-phone-matrix.md"
printf 'image\tclass\trole\tgeometry\tdata_decode\tbuild43_attempted\tbuild43_frozen\tbuild43_qualified\tbuild43_authenticated\tbuild64_attempted\tbuild64_seeds\tbuild64_geometry_evals\tbuild64_bank\tbuild64_qualified\tbuild64_decode_candidates\tbuild64_list_frames\tbuild64_authenticated\tbuild65_attempted\tbuild65_workers\tbuild66_attempted\tbuild66_workers\thmac\tpayload_match\ttelemetry_equivalent\tqualification\texit_code\telapsed_ms\tbuild65_elapsed_ms\tspeedup_vs_build65\n' > "$tsv"
...
for spec in "${specs[@]}"; do
    ...
    if [[ "$exp_evals" -gt 0 ]]; then
        if [[ "$b64_attempted" != true || "$b64_seeds" -ne 24 || "$b64_evals" -ne "$exp_evals" || "$b64_bank" -ne "$exp_bank" || "$b64_qualified" -ne "$exp_qualified" || "$b64_decode" -ne "$exp_decode" || "$b64_frames" -ne "$exp_frames" || "$b65_attempted" != true || "$b65_workers" -lt 1 || "$b66_attempted" != true ]]; then
            telemetry_equivalent=false
        fi
        if [[ "$exp_qualified" -gt 0 && "$b66_workers" -lt 1 ]]; then telemetry_equivalent=false; fi
        if [[ "$exp_qualified" -eq 0 && "$b66_workers" -ne 0 ]]; then telemetry_equivalent=false; fi
    else
        if [[ "$b64_attempted" != false || "$b65_attempted" != false || "$b66_attempted" != false ]]; then
            telemetry_equivalent=false
        fi
    fi
    ...
    if [[ "$role" == control ]]; then
        ...
        case "$role" in
            ...
            required-any)
                ...
                ;;
            required-build66)
                if [[ $rc -eq 0 && "$payload_match" == true && "$hmac" == true && "$b64_attempted" == true && "$b64_auth" == true && "$b65_attempted" == true && "$b66_attempted" == true && "$b66_workers" -gt 0 ]]; then qualification='PASS'; else qualification='FAIL'; fi
                ;;
            required-reject)
                if [[ $rc -ne 0 && "$hmac" == false && "$b64_attempted" == true && "$b64_auth" == false && "$b65_attempted" == true && "$b66_attempted" == true ]]; then qualification='PASS'; else qualification='FAIL'; fi
                ;;
        esac
    fi
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$file" "$class" "$role" "$geometry" "$data_decode" "$b43_attempted" "$b43_frozen" "$b43_qualified" "$b43_auth" "$b64_attempted" "$b64_seeds" "$b64_evals" "$b64_bank" "$b64_qualified" "$b64_decode" "$b64_frames" "$b64_auth" "$b65_attempted" "$b65_workers" "$b66_attempted" "$b66_workers" "$hmac" "$payload_match" "$telemetry_equivalent" "$qualification" "$rc" "$elapsed_ms" "$baseline_ms" "$speedup" >> "$tsv"
done

{
    echo '# PixSeal Build66 ordered parallel decode equivalence/performance matrix'
    echo
    echo 'Build66 preserves the qualified Build64 semantics exactly, keeps Build65 seed-parallel geometry, and additionally evaluates qualified protected-data candidates in deterministic ordered parallel batches.'
    echo
    echo '| image | role | geometry | B43 auth | B64 evals | B64 bank | B64 qual | B64 decode | B64 frames | B65 seed workers | B66 decode workers | HMAC | payload | telemetry eq | ms | B65 ms | speedup | qualification |'
    echo '|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|'
    tail -n +2 "$tsv" | while IFS=$'\t' read -r image class role geometry data_decode b43_attempted b43_frozen b43_qualified b43_auth b64_attempted b64_seeds b64_evals b64_bank b64_qualified b64_decode b64_frames b64_auth b65_attempted b65_workers b66_attempted b66_workers hmac payload_match telemetry_equivalent qualification rc elapsed_ms baseline_ms speedup; do
        echo "| $image | $role | $geometry | $b43_auth | $b64_evals | $b64_bank | $b64_qualified | $b64_decode | $b64_frames | $b65_workers | $b66_workers | $hmac | $payload_match | $telemetry_equivalent | $elapsed_ms | $baseline_ms | $speedup | $qualification |"
    done
    echo
    echo 'Required gate: complete Build64 outcome equivalence, exact deep-bank/evaluation/logical-decode telemetry on every fallback case, Build65 seed-parallel and Build66 ordered-decode telemetry on every deep fallback, and the nine-photo qualification matrix remains PASS. Timing and speedup are reported separately and do not affect correctness.'
} > "$md"

rm -f "$OUTPUT_DIR/logs/"*.payload.bin

control_passes="$(awk -F '\t' 'NR>1 && $3=="control" && $25=="PASS" {n++} END{print n+0}' "$tsv")"
a_front_pass="$(awk -F '\t' 'NR>1 && $1=="phone-a-front.jpg" && $25=="PASS" {n++} END{print n+0}' "$tsv")"
a_mild_pass="$(awk -F '\t' 'NR>1 && $1=="phone-a-mild.jpg" && $25=="PASS" {n++} END{print n+0}' "$tsv")"
a_angle_pass="$(awk -F '\t' 'NR>1 && $1=="phone-a-angle.jpg" && $25=="PASS" {n++} END{print n+0}' "$tsv")"
b_front_pass="$(awk -F '\t' 'NR>1 && $1=="phone-b-front.jpg" && $25=="PASS" {n++} END{print n+0}' "$tsv")"
b_mild_pass="$(awk -F '\t' 'NR>1 && $1=="phone-b-mild.jpg" && $25=="PASS" {n++} END{print n+0}' "$tsv")"
b_angle_pass="$(awk -F '\t' 'NR>1 && $1=="phone-b-angle.jpg" && $25=="PASS" {n++} END{print n+0}' "$tsv")"

echo "Build66 staged result: controls=${control_passes}/3 A/front=${a_front_pass}/1 A/mild=${a_mild_pass}/1 A/angle=${a_angle_pass}/1 B/front=${b_front_pass}/1 B/mild=${b_mild_pass}/1 B/angle-reject=${b_angle_pass}/1"
echo "Build66 equivalence/performance phone matrix written to:"
echo "  $tsv"
echo "  $md"

if (( control_passes != 3 || a_front_pass != 1 || a_mild_pass != 1 || a_angle_pass != 1 || b_front_pass != 1 || b_mild_pass != 1 || b_angle_pass != 1 )); then
    echo 'error: Build66 equivalence physical gate not met' >&2
    exit 1
fi
echo 'Build66 equivalence physical gate: PASS'
