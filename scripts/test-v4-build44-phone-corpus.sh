...
OUTPUT_DIR="${V4_PHONE_BUILD44_DIAGNOSTIC_DIR:-v4-phone private/build44-diagnostics}"
...
tsv="$OUTPUT_DIR/build44-phone-matrix.tsv"
md="$OUTPUT_DIR/build44-phone-matrix.md"
...
{
    echo '# PixSeal Build44 Go 1.26 smartphone qualification matrix'
    echo
    echo 'Build44 preserves the complete Build43 watermark/geometry/data path and changes only JPEG ingest to the deterministic PixSeal decoder.'
    echo
    ...
    echo
    echo 'Required gate: controls reject; A/front still authenticates through the unchanged Build43 geometry fallback; A/mild, A/angle and B/front remain authenticated; B/mild/B-angle are informational.'
} > "$md"
...
echo "Build44 staged result: controls=${control_passes}/3 A/front=${a_front_pass}/1 A/mild=${a_mild_pass}/1 A/angle=${a_angle_pass}/1 B/front=${b_front_pass}/1"
echo "Build44 phone qualification matrix written to:"
echo "  $tsv"
echo "  $md"

if (( control_passes != 3 || a_front_pass != 1 || a_mild_pass != 1 || a_angle_pass != 1 || b_front_pass != 1 )); then
    echo 'error: Build44 Go 1.26 physical gate not met' >&2
    exit 1
fi
echo 'Build44 Go 1.26 physical gate: PASS'
