...
## Build51 host result -> Build52 fine-restart hypothesis — 2026-09-24

**Build51 observation.** The primary B/mild target is candidate 10 (`top+left`, pair rank 3, seed rank 3), 34.049 px mean corner error from the post-hoc SIFT/reference oracle before refinement. The unchanged `16,8,4,2,1` coordinate descent accepts 9 proposal-improving moves and ends at 43.168 px. Only 2 accepted moves reduce oracle error; 7 increase it. The best oracle point seen in the exact trace, 25.172 px at `corner-2-x +16`, is correctly rejected by the frozen proposal objective because proposal drops `0.201986 -> 0.076256`.

The fixed Build51 stencil contains a different and more actionable point: `corner-2-x +2` raises proposal `0.201986 -> 0.208953` (`+0.006966`) and lowers post-hoc oracle error `34.049 -> 32.442 px` (`-1.607`). This is an ordinary coordinate direction, so Build51 classifies B/mild as `optimizer-opportunity`. The reason the existing refiner misses that exact opportunity is scheduling/state dependence: accepted 4 px moves have already changed the current geometry before the standard coarse-to-fine run reaches the 2 px stage. B/angle contains no better-both stencil point; its accepted path only worsens an already ~5.45 kpx false basin, so it remains informational and `proposal-surface-misaligned`.

**Build52 hypothesis.** Before changing the proposal observable, test whether an independent fine restart from the untouched seed can preserve the Build51 better-both neighborhood. For every blind top4 seed, run the unchanged Build41 coarse-to-fine baseline and, independently, a bounded `2 px -> 1 px` proposal-only coordinate descent from the untouched seed. Retain the seed plus every accepted fine-restart intermediate state rather than only the final greedy endpoint.

**Leakage barrier.** The seed set, all baseline endpoints and all fine-restart intermediate geometry must be generated without held-out folds, secret key, HMAC or reference/oracle information. Freeze the complete bank first. Only then annotate held-out/full-pilot qualification and diagnostic HMAC. Generate SIFT/reference geometry after blind B/mild and B/angle outputs exist and use it only for post-hoc error/classification.

**Decision rule.** B/mild is the primary target. If a retained fine state survives public qualification and authenticates, classify the experiment as optimizer recovery and only then consider a separately gated production design. If retained fine states reproduce a better-both geometric gain but do not authenticate, classify as partial optimizer gain and continue geometry research without weakening thresholds. If no retained fine state improves the target under the unchanged proposal objective, return to public-observable design. B/angle cannot by itself justify a production change.


## Build52 host result -> Build53 coupled pair-escape hypothesis — 2026-09-24

**Build52 result.** The qualified Go 1.26.0 host confirms a partial optimizer gain on B/mild. The 34.049 px target seed produces 8 retained fine states. State 1 (`corner-2-x +2`) is held-out-qualified at 32.442 px; state 2 improves proposal and post-hoc oracle further to 31.747 px but is not qualified. No retained state authenticates. B/mild is therefore `optimizer-partial-gain`. B/angle has no better-both retained state and is `optimizer-no-gain`.

**Design probe, not qualification evidence.** Replaying the B/mild qualified state locally with the deterministic JPEG path shows zero proposal-improving individual +/-1px coordinate moves. A complete coupled +/-1px two-coordinate stencil nevertheless contains four proposal-improving pair moves; all four pass the unchanged held-out qualification in the local preflight. The pair `corner-1-y -1` + `corner-2-x +1` raises proposal from 0.208953 to 0.210454 and reduces post-hoc oracle error from 32.442 to ~31.842 px. HMAC remains false. The B/angle target has no 1px coordinate-local 2px root, so the proposed pair scan does not trigger on that target. These observations motivate the experiment but are not treated as host-qualified results.

**Build53 hypothesis.** The remaining B/mild optimizer failure may include coordinate-wise local maxima: the unchanged proposal objective can reject every individual 1px move even when a coupled two-coordinate 1px move improves the same objective. Build53 will preserve the top4 seed bank and score, derive the 2px accepted-state roots, probe each root with all 16 individual +/-1px coordinate neighbors, and only when zero improve proposal evaluate the 112 deterministic coupled two-coordinate +/-1px combinations. At most eight proposal-improving pair states per root are retained by proposal rank alone.

**Leakage barrier.** Root creation, locality classification, pair generation and pair ranking are proposal-only. The complete geometry bank for both B/mild and B/angle must be frozen before held-out/full-pilot qualification or diagnostic HMAC. SIFT/reference geometry is generated only after both blind JSON files exist and is post-hoc only. No production score, threshold, quorum, encoder, pilot, ECC/Hamming or HMAC domain changes in Build53.


## Build53 host result -> Build54 post-pair continuation hypothesis — 2026-09-24

**Build53 result.** On B/mild candidate 10, the qualified 32.442 px root is 1px coordinate-local. Build53 retains four coupled proposal-improving pair states; all four pass unchanged held-out qualification. Pair rank 2 improves proposal to 0.210454 and post-hoc oracle error to 31.842 px. No pair state authenticates, so the result is `pair-qualified-gain`. The B/angle target has no coordinate-local root and is `pair-not-triggered`.

**Design probe, not qualification evidence.** Continuing all four frozen B/mild pair states with ordinary 1px proposal-only coordinate descent shows that pair rank 1 is not itself a terminal proposal basin. The branch admits multiple accepted moves; an intermediate remains qualified and reaches about 30.539 px before later proposal ascent worsens oracle error. No continuation HMAC succeeds.
...
## Build59 host result -> Build60 post-third-pair continuation — 2026-09-24

**Observation.** Build59 confirms `third-pair-qualified-gain` on B/mild without changing the proposal score. The blind B/mild bank contains 180 third-pair states, 175 qualified and 0 authenticated. Target candidate 10 reaches 25.407 px post-hoc oracle error from its exact 27.071 px Build58 sibling parent while proposal is also improved. B/angle target candidate 18 remains not triggered; its full third-pair bank contains 914 states, none qualified or authenticated.

**Artifact note.** The original Build59 archive preserves the valid B/mild blind JSON; a later oracle-enabled rerun accidentally truncated that one JSON file after reports had already been produced. A reconciled artifact combines the original blind JSON with the later oracle/report files and matches the recorded state/qualification counts exactly.

**Build60 rule.** Do not select the 25.407 px state using oracle, qualification or HMAC. Reproduce the complete Build59 blind bank and continue every retained third-pair state with the established 1px proposal-only coordinate descent for at most eight passes. Retain every accepted intermediate. Freeze the complete B/mild+B/angle continuation bank before qualification/HMAC and generate SIFT/reference only after both blind JSON outputs exist.



## Build60 host result -> Build61 post-third-pair sibling stencil — 2026-09-24

**Observation.** Build60 reaches a qualified 25.150 px B/mild continuation state and retains 397/406 qualified B/mild continuation states with no HMAC. The exact parent of the best state is 29.938 px. B/angle target remains not triggered, but 2/1467 non-target continuation states now pass qualification around 5665 px and still fail HMAC.

**Decision.** Continue optimizer-only for one more directly motivated test: evaluate independent +/-1px siblings from every Build60 continuation parent, exactly as prior sibling-stencil builds removed Gauss-Seidel order dependence after continuation stages. Do not oracle-select the 25.150 px state.

**Build61 rule.** Generate and rank sibling geometry only with the unchanged proposal objective, freeze complete B/mild+B/angle banks, then attach qualification/HMAC and finally post-hoc oracle. Track whole-bank B/angle qualification rate explicitly; do not interpret qualification without HMAC and oracle proximity as recovery.


## Build61 host result -> Build62 fourth-pair escape — 2026-09-24

**Observation.** Build61 reaches a qualified 24.614 px B/mild sibling. Its exact Build60 continuation parent is 25.359 px, giving a 0.745 px oracle improvement while proposal rises to 0.288064. B/mild freezes 593 siblings total, 591 qualified and 0 authenticated. B/angle freezes 1902 siblings, 0 qualified and 0 authenticated; the two non-target qualified continuation states seen in Build60 therefore do not survive as qualified Build61 siblings.

**Decision.** Continue optimizer-only with the symmetric pair-escape test already validated at earlier sibling stages. Do not select the 24.614 px state using oracle. Probe every frozen Build61 sibling for one-coordinate locality using the unchanged proposal objective, then apply the bounded pair stencil only to proposal-local parents.

**Build62 rule.** Generate and rank fourth-pair geometry only with proposal, freeze the complete B/mild+B/angle bank, then attach qualification/HMAC and finally post-hoc oracle. Continue tracking whole-bank B/angle qualification and authentication explicitly.


## Build62 host result -> Build63 post-fourth-pair continuation — 2026-09-24

**Observation.** Build62 confirms `fourth-pair-qualified-gain` on B/mild: 492 fourth-pair states are frozen, 491 qualify and none authenticate. The best target state reaches 24.056 px from its exact 25.641 px parent. B/angle freezes 3746 fourth-pair states and none qualifies or authenticates.
...
**Interpretation.** This closes the immediate optimizer research objective: a geometry bank created entirely from public proposal evidence can reach an independently qualified state that authenticates the protected frame. HMAC did not guide geometry; it identifies one valid frame only after the complete bank was frozen and qualified. Oracle distance is not an adequate selector because the HMAC-positive state is not the oracle minimum.
