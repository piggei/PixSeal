...
Performance is now strong enough to promote the scheduling change: B/mild fell from 828036 ms to 544038 ms (-34.3%, 1.522x), while the complete nine-photo matrix fell from 2372255 ms to 2175687 ms (-8.3%, 1.090x). Small regressions on control/mild and B/angle do not change semantics and occur where ordered parallel protected-data decode has little or no useful work; the expensive authenticated decode path is materially faster. Build66 therefore supersedes Build64 as the current qualified smartphone baseline, while Build64 remains the historical deep-recovery qualification checkpoint.

## Build65 — deterministic seed-parallel deep-recovery candidate

After Build64 became the qualified smartphone baseline, Build65 tested scheduling-only parallelism across the 24 already-selected proposal-only seed branches. The Go 1.26.0 host gate passed the complete nine-photo matrix and reproduced every Build64 deep-recovery seed/evaluation/bank/qualification/decode/list-frame count exactly. The candidate used 8 seed workers whenever deep recovery ran. It is intentionally not promoted: Build64 had no matched `elapsed_ms` artifact for a rigorous same-host baseline, and B/mild still required 828036 ms in Build65, showing that serial protected-data decode remains a dominant cost. Build64 therefore remains the baseline while Build66 targets that decode stage without altering logical order.

## v0.3.0-build64 — qualified deep phone recovery baseline

Build63 is the first research snapshot to recover retained B/mild blindly with a valid HMAC. Its final continuation bank contains 937 states, 935 qualified and one authenticated. The authenticated geometry is 28.868 px from the independent post-hoc oracle and is not the geometry-minimum state, reinforcing the separation between oracle analysis and blind geometry selection. B/angle contains 6198 continuation states with no qualified or authenticated state. Build64 stops the optimizer-only expansion and promotes the exact Build63 search family as a final additive production fallback. Existing Build44 paths remain first and unchanged; the deep bank is generated and frozen before qualification/HMAC. The complete retained nine-photo physical gate passes under Go 1.26.0: controls reject, all historical Build44 PASS cases remain on their old paths, B/mild authenticates through Build64 and B/angle remains rejected. Build64 therefore supersedes Build44 as the qualified smartphone physical-recovery baseline.

## v0.3.0-build63 — post-fourth-pair continuation

Build62 confirmed that the unchanged proposal objective still exposes useful geometry: B/mild freezes 492 fourth-pair states, 491 qualify and none authenticate. The best qualified target state reaches 24.056 px from its exact 25.641 px parent, while B/angle freezes 3746 fourth-pair states with 0 qualified and 0 authenticated. Build63 remains optimizer-only: it continues every frozen Build62 fourth-pair state with bounded 1px proposal-only coordinate descent and retains every accepted intermediate. Qualification, HMAC and oracle remain strictly after the complete geometry freeze. At this research checkpoint, Build44 was the qualified production baseline.

## v0.3.0-build62 — fourth-pair escape from proposal-local Build61 siblings

Build61 confirmed that the unchanged proposal objective still exposes useful geometry after the third-pair continuation: B/mild freezes 593 proposal-improving siblings, 591 qualify and none authenticate. The best qualified sibling reaches 24.614 px from its exact 25.359 px parent, a 0.745 px geometric improvement, while proposal also rises. B/angle freezes 1902 siblings with 0 qualified and 0 authenticated, so the small qualification leakage first seen in Build60 does not propagate through the Build61 sibling stage. Build62 remains optimizer-only: it tests every frozen Build61 sibling for one-coordinate +/-1px locality using proposal only and applies the established bounded pair stencil only to proposal-local siblings. Qualification, HMAC and oracle remain strictly after the complete geometry freeze. At this research checkpoint, Build44 was the qualified production baseline.

## v0.3.0-build61 — post-third-pair continuation sibling stencil

Build60 confirmed that the unchanged proposal objective still exposes useful geometry after the third pair escape: B/mild produced 406 continuation states, 397 qualified and 0 authenticated, with the best qualified state at 25.150 px. The target B/angle path remained not triggered, but the expanded non-target B/angle bank produced two qualified continuation states at roughly 5665 px and still no HMAC. Build61 therefore stays optimizer-only but adds an explicit full-bank qualification-rate control. It reproduces the complete Build60 blind bank and evaluates every independent +/-1px coordinate sibling from every retained post-third-pair continuation state against the identical frozen parent. Qualification, HMAC and oracle remain strictly after the geometry freeze. At this research checkpoint, Build44 was the qualified production baseline.

## v0.3.0-build60 — post-third-pair continuation

Build59 confirmed that the unchanged proposal objective still contains useful geometry beyond the Build58 sibling bank: B/mild retains 180 third-pair states, 175 qualified and 0 authenticated, with the best qualified target state at 25.407 px. Build60 therefore makes no score or production change. It reproduces the full blind Build59 bank and continues every retained third-pair state with bounded 1px proposal-only coordinate descent, retaining every accepted intermediate before qualification/HMAC. At this research checkpoint, Build44 was the qualified production baseline.

## Build59 — third pair escape from proposal-local Build58 siblings

The qualified Build58 host artifact closes B/mild as `second-pair-sibling-qualified-gain`: 275 sibling states are frozen, 271 qualify, none authenticate, and target candidate 10 improves from 26.968 px to 26.149 px while proposal also rises. B/angle freezes 542 siblings with no qualified state. Build59 reproduces the complete Build58 blind bank, tests every frozen second-pair sibling against the complete independent +/-1px one-coordinate stencil, and applies the bounded coupled pair stencil only at proposal-local siblings. The qualified Build59 host artifact then closes B/mild as `third-pair-qualified-gain`: 180 third-pair states are frozen, 175 qualify, none authenticate, and the best qualified target geometry reaches 25.407 px. B/angle target remains not triggered; its complete third-pair bank contains 914 states and none qualifies. Build59 is research-only; At this research checkpoint, Build44 was the latest qualified production milestone.

## Build58 — sibling stencil after second-pair continuation

The qualified Build57 host artifact closes B/mild as `second-pair-continuation-qualified-gain`: target candidate 10 has 154 continuation states, 147 qualified, and no HMAC; the best qualified intermediate reaches 26.968 px. B/angle target remains not triggered and its complete continuation bank has no qualified state. Build58 reproduces the full Build57 blind bank and evaluates all 16 independent +/-1px coordinate siblings from every retained post-second-pair continuation state against the same frozen parent. The expanded bank is frozen before qualification/HMAC and oracle geometry remains post-hoc. Build58 is research-only; At this research checkpoint, Build44 was the latest qualified production milestone.
...
A deterministic post-hoc design probe shows that the 29.778 px sibling has zero proposal-improving independent +/-1px single-coordinate neighbors, yet a bounded coupled two-coordinate stencil contains five proposal-improving states. One retained pair reaches about 28.514 px while remaining qualified; HMAC is still false. Build56 therefore reproduces the complete Build55 blind bank, classifies sibling locality using proposal only, and applies the 112-combination pair stencil only to proposal-local siblings. At most eight pair states are retained by proposal. Geometry is frozen before qualification/HMAC and SIFT/reference remains post-hoc. Build56 is research-only; At this research checkpoint, Build44 was the latest qualified production milestone.

## Build55 — continuation sibling-stencil observability

The qualified Build54 host run closes B/mild as `continuation-qualified-gain`: candidate 10 retains 34 continuation states and reaches a held-out-qualified intermediate at 30.539 px, 1.908 px better than its pair-rank-1 parent. HMAC remains false. The B/angle target remains `continuation-not-triggered`. Build55 freezes each continuation parent and evaluates the complete independent 16-point +/-1px sibling stencil from that exact parent, eliminating Gauss-Seidel coordinate-order dependence while keeping the proposal score unchanged.

The qualified Go 1.26.0 Build55 host artifact confirms the hypothesis. B/mild target candidate 10 retains 91 proposal-improving siblings and the best qualified sibling reaches 29.778 px while proposal improves `0.257772 -> 0.266832`; HMAC remains false. B/angle target remains `sibling-not-triggered`. Across the complete blind banks, B/mild freezes 93 siblings (76 qualified, 0 authenticated) and B/angle freezes 106 (0 qualified, 0 authenticated). Build55 is research-only; At this research checkpoint, Build44 was the latest qualified production milestone.
...
Build54 keeps that entire blind mechanism fixed and asks what happens **after** the coupled escape. Every retained Build53 pair state is continued with bounded 1px proposal-only coordinate descent for at most eight passes, and every accepted intermediate is retained. The complete continuation bank is frozen before held-out/full-pilot qualification, protected-data decode or HMAC; SIFT/reference geometry remains post-hoc. The qualified Go 1.26.0 host run confirms `continuation-qualified-gain`: B/mild retains 34 continuation states and reaches a qualified 30.539 px intermediate, but no continuation state authenticates; B/angle remains `continuation-not-triggered`. Build54 is research-only and, at this research checkpoint, Build44 was the latest qualified production milestone.
...
For every unchanged top4 seed, Build53 retains the proposal-only 2px path, probes each root with the complete single-coordinate +/-1px stencil, and enables a 112-combination coupled +/-1px pair stencil only when the root has zero single-coordinate improvements. Up to eight pair states are kept by proposal rank alone. All geometry is fixed before held-out/full-pilot qualification, protected-data decode or HMAC; reference/SIFT remains post-hoc. Build53 is research-only and, at this research checkpoint, Build44 was the latest qualified production milestone.
...
The complete baseline/restart bank is generated blind and frozen before held-out/full-pilot qualification. Key/HMAC is used only after qualification and the SIFT/reference geometry remains external and post-hoc. Build52 is research-only; At that checkpoint, Build44 was the latest qualified production milestone and no encoder, Format-v4 field, pilot, strength, ECC/Hamming, whitening/HMAC domain, geometry threshold or quorum is changed.
...
The qualification-host Build49 run resolved the Build48 seed-selection ambiguity. The B/mild oracle-nearest Build47 candidate remains 34.049 px from the reference and is raw-proposal rank 3 within its side pair (global rank 17). Fold-min, balanced-fold and tile-consistency observables rank it 7, 12 and 13 respectively, so none improves on the existing proposal score. Build50 therefore changes no score: it extends only the diagnostic local-refinement seed depth from top2 to top4 per side pair. The nested top2/top4 comparison is extracted from one blind top4 run; held-out qualification, diagnostic HMAC and post-hoc SIFT/reference error remain strictly downstream of frozen geometry. At this research checkpoint, Build44 was the latest qualified milestone.
...
Build52 preserves the Build50/51 top4 seed bank and unchanged proposal score. For every untouched seed it records the normal Build41 coarse-to-fine endpoint and independently runs a bounded `2 px -> 1 px` proposal-only restart, retaining the seed plus every accepted intermediate state. The entire baseline/restart geometry bank is frozen before held-out/full-pilot qualification; diagnostic HMAC is downstream of qualification and SIFT/reference error remains external/post-hoc. Build52 is research-only and, at this research checkpoint, Build44 was the latest qualified milestone.
